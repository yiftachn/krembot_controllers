...
public:
     ...
     */
    BoundedPQueue<ElemType>* kNNNeighbours(const Point<N>& key, std::size_t k);
...
void KDTree<N, ElemType>::nearestNeighborRecurse(const typename KDTree<N, ElemType>::Node* currNode, const Point<N>& key, BoundedPQueue<ElemType>& pQueue) const {
    ...
    int _height = 250;
    int _width = 250;
    m_cOutput.open("~/krembot_ws/check.txt", ios_base::trunc | ios_base::out);
    m_cOutput << "curr node is null" << '\n';
    m_cOutput.close();
    if (currNode == NULL) return;
    const Point<N>& currPoint = currNode->point;
    m_cOutput.open("~/krembot_ws/check.txt", ios_base::trunc | ios_base::out);
    m_cOutput << "curr node is null" << currPoint[0] << ',' << currPoint[1] << '\n';
    m_cOutput.close();
...
template <std::size_t N, typename ElemType>
BoundedPQueue<ElemType>* KDTree<N, ElemType>::kNNNeighbours(const Point<N>& key, std::size_t k){
    auto* bpq = new BoundedPQueue<ElemType>(k);
